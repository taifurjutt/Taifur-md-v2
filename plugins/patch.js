  ...
  async (message, args) => {
    try {
      ...
      if (url.toLowerCase().includes("https")) {
        ...
        return await message.send("`taif-MD UPDATE WAS SUCCESSFULL ✅`");
      } else {
...
astro_patch.smd(
  ...
  async (context) => {
    ...
    const { key: messageKey } = await context.reply("*wait...*");
  ...
  async (m) => {
    try {
      ...
      const contextInfo = {
        isForwarded: true,
        forwardingScore: 999,
        title: "taif MD Running Since",
        body: message,
        footerText: "taif MD 2024",
